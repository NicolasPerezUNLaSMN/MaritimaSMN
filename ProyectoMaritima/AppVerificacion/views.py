...
def fecha_hoy(request):
    # Obtener las fechas de inicio y fin directamente
    fecha_inicio = datetime.date.today().strftime('%Y-%m-%d')
    fecha_fin = (datetime.date.today() + datetime.timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    where_clause = f"fecha >= '{fecha_inicio} 00:00:00' AND fecha < '{fecha_fin} 00:00:00'"
    ...
    return render(request, 'AppVerificacion/reporte_barcos_hoy.html', {'barcos': barcos})
